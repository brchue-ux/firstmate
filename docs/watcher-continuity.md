...
A live owner has one proven exception, because a session that the harness re-hosts into another process is otherwise indistinguishable from a competing one and would leave the home unsupervised for the rest of that session.
`bin/fm-session-lock-lib.sh` owns the re-host record and the proof it requires, `bin/fm-lock.sh --adopt-session` owns the re-anchor, and [`turnend-guard.md`](turnend-guard.md) states the contract.
...
An attached arm follows verified identity-matched successors, but its outcome depends on whether supervision is genuinely absent rather than on whether that arm personally saw the wake reason, because the owning arm consumes and prints it.
...
That adapter-side classification is older than this arm-layer rule, and aligning those two adapters with it is separate work.
...
`--restart --force` remains available for a deliberate replacement.
Pi's and OpenCode's `--restart` re-arm from a child close is unaffected, because their previous watcher has already exited by then.

The arm layer appends one tab-separated record per observed cycle to `state/.watch-cycle-exits.log`.
Each record includes arm and watcher PIDs, start and end timestamps, exit code and signal, classified reason, beacon age, lock identity before and after close, and successor disposition.
The file is size-capped through `FM_WATCH_CYCLE_LOG_MAX_BYTES` and `FM_WATCH_CYCLE_LOG_KEEP_LINES`.
`state/.watch-triage.log` remains the watcher's own bounded debug log and carries no lifecycle semantics.
...
It also covers the attached arm's outcome contract: two arms plus one actionable wake produce no failure from the attached arm, an idle home closes quietly, a foreign auto-arm claim accounts for a cycle end while this arm's own launcher does not, and `--restart` declines a verified healthy watcher unless `--force` is passed.
`tests/fm-subagent-pretool-check.test.sh` proves Claude retains only the non-status Bash seatbelts.
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight claimed ahead of stale-lock reclaim so the turn-end guard can see an in-progress recovery during a slow reclaim, and exit-2 translation.
