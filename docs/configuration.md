...
Because a backend endpoint id is reusable, `fm-send.sh` additionally names both records and refuses - on the task-id path and on the explicit-endpoint path alike, before any backend is consulted - when two live task records in this home resolve to the same endpoint, rather than typing one task's message into another's session.
...
Setting `FM_HOME_BINDING` to the same home as `FM_HOME` declares the selection deliberate and is accepted as given; every genuine cross-home call in `bin/` sets it per invocation, alongside the `FM_HOME` it is declaring.
Resolution reads that declaration and never issues one of its own, so a binding reaches only the command tree its caller handed it to and can never be picked up by an unrelated descendant of a process that merely resolved a home.
...
Within one process the rule is idempotent: resolution records the home it settled on and reads that record back when a script's sourced libraries resolve again, so a command whose code root is a different home root from the one it is standing in never refuses the home it assigned itself.
That record is a non-exported shell variable bound to the resolving process, it is honored only while `FM_HOME` still names the same home, and a same-named value arriving in the environment is never read as a decision the process made - so it is a within-process detail and never a way to declare a home.
...
The v1.31.2 no-mistakes minimum is a capability floor rather than a currency check, so the first mate separately reports an install that clears the floor but is behind the current release, reusing no-mistakes' own upgrade banner instead of querying a release feed.
That report is advice only: it never updates anything on its own, and upgrading stays your call like any other install.
Because that banner is a cached advisory no-mistakes refreshes on its own schedule, the report is best-effort, so its absence does not prove your install is current.
The version it names as installed is read from the binary rather than from that cache, and it reports nothing unless the advertised release is genuinely newer, so upgrading outside the first mate cannot leave it advertising a release you already run.
...
The locked session-start bootstrap step also runs the guarded local secondmate sync for recorded secondmate homes, then propagates declared inherited local material into each validated home, including a home whose agent the liveness sweep left down.
...
The same bootstrap run emits `SECONDMATE_LIVENESS:` only when a registered secondmate is skipped or its relaunch fails; already-live, successfully relaunched, and correctly-left-down-idle secondmates are handled silently.
A dead or missing secondmate is relaunched only when its own home shows pending work; see [`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md) "Recovery" for the pending-work test that fleet startup applies.
...
FM_IDLE_SWEEP_BUDGET_SECS=120        # seconds the heartbeat cleanup sweep may spend before deferring the rest to the next heartbeat; 0 removes the limit
FM_IDLE_SWEEP_TASK_TIMEOUT_SECS=60   # seconds allowed per cleanup attempt in that sweep; 0 removes the cap
FM_IDLE_SWEEP_RETRY_SECS=3600        # seconds before that sweep retries a refused task whose status log and metadata have not moved
FM_IDLE_SWEEP_TEARDOWN_BIN=bin/fm-teardown.sh   # test override for the cleanup entrypoint the sweep offers finished tasks to
...
FM_GUARD_ESCALATE_LADDER='300 900 3600'   # outage durations, in seconds, at which fm-guard re-prints the full watcher-down banner instead of a reminder
FM_GUARD_ESCALATE_REPEAT=3600   # seconds between fm-guard's full-banner re-prints once an outage passes the last ladder rung
