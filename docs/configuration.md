...
The same bootstrap run emits `SECONDMATE_LIVENESS:` only when a registered secondmate is skipped; already-live and left-down secondmates are handled silently.
A dead or missing secondmate is never relaunched at session start, regardless of whether its own home shows pending work; see [`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md) "Recovery" for the pending-work check, which now only feeds an FYI fact, never a relaunch decision.
