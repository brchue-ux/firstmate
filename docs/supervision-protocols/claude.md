...
5. Manual arm is recovery only.
   When a repair is genuinely needed - the Stop hook did not claim this home, or a live watcher must be deliberately replaced - run `bin/fm-watch-arm.sh` (or `bin/fm-watch-arm.sh --restart`) as its own Claude Code background task, never bundled with other commands, never with shell `&`.
