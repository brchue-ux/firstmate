...
| `fm-browser-mcp-pin.sh`  | Resolve, and with `--ensure` install, the `chrome-devtools-mcp` build crewmate browser work is pinned to (docs/configuration.md) |
...
| `fm-fleet-work-index.sh` | Index every open backlog item across this home and every transitively registered secondmate home, read-only, naming any home it had to skip |
...
| `fm-test-run.sh`         | Behavior-test runner: selection, portable lanes, proven-isolated `--jobs`, coverage guard, timing/JSON, and fixture containment plus the orphan reap for the suite |
| `fm-test-isolation-proof.sh` | Concurrent isolation proof and proven-isolated candidate set owner |
| `fm-ensure-agents-md.sh` | Ensure a project's real `AGENTS.md`, its `CLAUDE.md` symlink, and the canonical self-governance section |
| `fm-guard.sh`            | Warn on primary-checkout tangles, shared-temp-filesystem pressure, pending queued wakes, and stale watcher liveness |
| `fm-primary-scope-lib.sh` | Shared marker-or-plain-checkout primary-home predicate for tracked hooks             |
| `fm-home-anchor-lib.sh`  | Single owner of `FM_HOME` resolution and its refusal on an ambiently inherited home (docs/configuration.md) |
...
| `fm-browser-session-lib.sh` | Single owner of the `chrome-devtools-axi` session name pinned to a task, shared by the brief, teardown, and the browser sweep |
...
| `fm-project-mode.sh`     | Resolve a project's delivery mode, `+yolo` flag, and `base=<remote>` from `data/projects.md` |
...
| `fm-backlog-parse-lib.sh` | Single owner of the `data/backlog.md` markdown parser shared by the fleet snapshot and the cross-home work index |
...
| `fm-treehouse-spelling-lib.sh` | Translate a resolved real worktree/home path to the spelling `treehouse`'s own pool registry recognizes, for `fm-teardown.sh` and `fm-home-seed.sh` |
...
| `fm-deskcap-mcp.py`      | Serve the MCP stdio surface that hands an agent a still PNG of the captain's desktop |
| `fm-deskcap-lib.py`      | Capture one desktop still to memory over the compositor's screen-cast API, or the desktop portal as fallback |
