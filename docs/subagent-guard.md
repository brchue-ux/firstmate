...
Every tracked primary-scoped hook answers "is this a guarded primary" through that one predicate - the session-start nudge, the turn-end guard, the Claude stop autoarm, and the cd-guard ([`cd-guard.md`](cd-guard.md)) - so their scopes cannot drift apart.
