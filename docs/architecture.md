...
That bounded recheck names a confirmed-dead crew as such rather than as an ordinary continuing external wait, on every classification path and whatever `bin/fm-crew-state.sh` reports, because the live agent verdict alone decides it.
