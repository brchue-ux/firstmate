...
That index answers on a bounded call and succeeds on a partial read by design, so the sweep treats an unavailable answer and an incomplete one alike: any home reported skipped or whose secondmate subtree could not be enumerated produces one whole-sweep skip naming those homes, and nothing is flagged that run.
