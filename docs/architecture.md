...
The sweep resolves which sessions are still owned by deriving that name for every open item in `bin/fm-fleet-work-index.sh`, and session start runs it in the main home only, because that index walks strictly downward from the home running it while the bridge state it reads is host-global.
