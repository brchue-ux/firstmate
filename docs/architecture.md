...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, windows whose pane capture fails outright, heartbeat backstop hits, and heartbeats whose idle sweep actually reclaimed a finished task.
A pane that cannot be read at all is a stronger staleness signal than an unchanged one, so it surfaces once per failure spell - for a working, blocked, or paused crew alike - instead of dropping that window from every later poll, and clears the moment capture succeeds again.
...
That bounded recheck names a confirmed-dead crew as such rather than as an ordinary continuing external wait, on the full classification path and on the throttled polls between crew-state reads alike, because a live dead verdict outranks whatever `bin/fm-crew-state.sh` reported - except an actively running pipeline, which the full path absorbs as working before any agent verdict is read.
...
A tick that actually reclaims something folds the reclaimed ids into that heartbeat's single wake record, because the task's records are gone afterwards and its completion and newly unblocked work still have to be recorded, while skips and refusals go to the triage log alone so routine cleanup attempts never wake firstmate.
Each home's own watcher sweeps only that home's tasks.
...
One sweep-wide lock keeps two sweeps in a home from overlapping, so a second one stands down quietly, while `--dry-run` skips that lock because it reports candidates and changes nothing.
Exclusion between a sweep and a hand-run teardown of the same task is deliberately not closed here, and closing it would take a per-task lock inside `bin/fm-teardown.sh`, which is out of scope for this mechanism.
...
[`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the arm layer's successor, terminal-delivery, re-arm recovery, and typed clean-close failure contract, including why an attached arm reports that failure only when supervision is genuinely absent rather than whenever it personally missed the reason.
The arm layer records one bounded lifecycle row per observed cycle in `state/.watch-cycle-exits.log`; `state/.watch-triage.log` remains the watcher's own debug log, holding absorbed wakes and heartbeat-sweep outcomes, and never cycle-lifecycle records.
...
Its `--restart` mode signals only the watcher recorded in the current home's `state/.watch.lock`, so restarting one home cannot kill sibling secondmate watchers, and it declines to kill a verified healthy watcher unless `--force` is passed.
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if the shared temp filesystem is filling up, or if work, process-event sources, or Relay polling has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
It leads with a prominent bordered tangle banner, while `bin/fm-guard.sh` owns the watcher-down banner and reminder policy, which keys repetition on outage duration so a lengthening outage re-claims the full banner on an escalation ladder instead of going quiet after its first print.
The temp-filesystem alarm reuses that escalation shape on a different key: `bin/fm-tmp-usage.sh` owns the measurement and its warn/high/critical severity, the guard owns only loudness and re-claims the full banner at each higher severity, and `bin/fm-tmp-sweep.sh` stays the single owner of reclamation, so the guard warns about pressure and never removes anything itself.
Browser bridges are the one leaked resource no kill path can reach: `chrome-devtools-axi` detaches its bridge into its own session at startup, so it is in neither the task endpoint's process group nor its session and survives pane closure, worktree return, and worktree removal, holding a headless Chrome tree open behind it.
Cleanup therefore uses the tool's own `stop`, scoped to the session name `bin/fm-brief.sh` pins to the task, and `bin/fm-browser-sweep.sh` is the out-of-band backstop for workers that never reach cleanup - it only reports, because a bridge that looks idle may belong to a live worker in another home.
That session name has one owner, `bin/fm-browser-session-lib.sh`, shared by the brief that pins it, the cleanup that stops it, and the sweep that decides who still owns one, since a name spelled twice is a bridge that leaks or a live browser reported as an orphan.
The sweep resolves which sessions are still owned by deriving that name for every open item in `bin/fm-fleet-work-index.sh`, and then again for every live task record in each home that index reported, because the index knows a task by its backlog row while work dispatched straight from a conversation only ever has a runtime record.
Session start runs the sweep in the main home only, because that index walks strictly downward from the home running it while the bridge state it reads is host-global.
Run from a secondmate home it would return that home's own subtree as a confident complete answer and flag a sibling's live worker, so the fleet root sweeps on every home's behalf and `bin/fm-bootstrap.sh` skips it wherever a secondmate marker is present.
That index answers on a bounded call and succeeds on a partial read by design, so the sweep treats an unavailable answer and an incomplete one alike: any home whose open work the index could not determine produces one whole-sweep skip naming those homes, and nothing is flagged that run.
Which homes those are is the index's own call, carried per home as `work_unknown` so no consumer has to match its reason wording: an unreadable or unparseable backlog, a home that cannot be found, and an unenumerable secondmate registry are gaps, while a home with no backlog file at all or one already counted under an earlier registry id is a complete answer.
...
That workspace is anchored at the home directory itself and opened worktree-backed so secondmate homes render grouped under the primary in Herdr's sidebar, degrading to a flat ungrouped workspace rather than failing a spawn; [Home workspace grouping](herdr-backend.md#home-workspace-grouping) owns that contract.
Spawn-time task placement is launcher-bound rather than label-only: [Watching and task containers](herdr-backend.md#watching-and-task-containers) owns that placement, the label-only fallback, and recovery scope.
...
`data/projects.md` records each project's standing posture, optional `+yolo` flag, and optional `base=<remote>` base remote as the captain's default and as context for that decision, including the conditional `no-mistakes-prod-only` policy; a ship spawn that drops below the registered rigor prints a deviation notice and continues.
...
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip and base remote, and home seeding's refusal and no-mistakes initialization.
...
At session start, confirmed-dead secondmate agent endpoints are closed but never relaunched, regardless of whether that secondmate's own durable records show pending work (`secondmate-provisioning` "Recovery"); the captain reopens one manually, and ambiguous liveness reads are left untouched to avoid duplicate supervisors.
