...
At session start, confirmed-dead secondmate agent endpoints are closed; a relaunch through the same secondmate spawn path follows only when that secondmate's own durable records show pending work (`secondmate-provisioning` "Recovery"), while ambiguous liveness reads are left untouched to avoid duplicate supervisors.
