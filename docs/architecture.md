...
The sweep resolves which sessions are still owned by deriving that name for every open item in `bin/fm-fleet-work-index.sh`, so a worker in any secondmate home is protected fleet-wide rather than only in the home running the sweep; when that index cannot be consulted at all it reports one whole-sweep skip and flags nothing.
