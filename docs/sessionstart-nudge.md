...
Both resolve the home through `bin/fm-home-anchor-lib.sh` ([configuration.md](configuration.md#fm_home)) and stay silent when that resolution refuses, because they are only an enforcement layer for a digest the primary can still run itself, and the exit-0 rule below forbids a louder failure.
