...
That matters because `/tmp` is a tmpfs on some hosts, where a leaked fixture is leaked RAM rather than disk; the runner's header owns the reap's safety rules and its test-only environment seams.
