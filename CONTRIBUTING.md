...
- Helper scripts in `bin/` are plain bash, apart from a few Python 3 helpers whose API has no usable shell binding, such as the desktop-capture pair `bin/fm-deskcap-lib.py` and `bin/fm-deskcap-mcp.py` (see [`docs/desktop-capture.md`](docs/desktop-capture.md)).
  Each starts with a usage header comment; keep it accurate when you change behavior.
  Test scripts and helpers in `tests/` are plain bash too, alongside a few `<subject>.test.py` unit helpers, and `tests/*.test.sh` stays the discovery surface either way.
