...
- Changes to harness adapters (detection in `bin/fm-harness.sh`, launch and hook mechanics in `bin/fm-spawn.sh`, busy signatures in `bin/fm-watch.sh` and `bin/fm-tmux-lib.sh`, cleanup in `bin/fm-teardown.sh`, and per-harness facts in `.agents/skills/harness-adapters/adapters/<harness>.md`) must be verified empirically against the real harness, never written from documentation alone.
