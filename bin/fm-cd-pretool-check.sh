...
if [ "$CMD_SET" -eq 0 ]; then
  ...
  # A missing shared library is inert here for the same reason every other
  # failure in this guard is: it never denies a command it cannot classify.
  HOOK_HOST_LIB="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)/fm-hook-host-lib.sh"
  [ -f "$HOOK_HOST_LIB" ] || exit 0
