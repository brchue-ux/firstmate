...
# one owner: bin/fm-home-anchor-lib.sh. This hook runs the digest - which drains
# the wake queue and runs the mutating bootstrap sweeps - so an inherited FM_HOME
# naming a different home would take that home's session lock and edit its
# durable records. A refusal is silent here for the same reason every other exit
# below is: this is only an enforcement layer for a digest the primary can still
# run itself, and the exit-0 rule forbids a louder failure.
