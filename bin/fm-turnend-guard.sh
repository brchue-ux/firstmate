...
if [ "$COUNT" -gt "$BLOCK_BUDGET" ]; then
  ...
  printf '{"systemMessage":"firstmate turn-end guard: %s with no live watcher and no Stop auto-arm claim; block budget exhausted, allowing this stop. Repair supervision (bin/fm-watch-arm.sh as a Claude Code background task, which attaches instead of disturbing a watcher that is already healthy) or investigate why bin/fm-claude-stop-autoarm.sh is not claiming this home."}\n' "$NEED_DESC"
  exit 0
fi
