...
phase_label() {  # <phases>
  case "$1" in
    ...
    probe,sweeps) printf 'GitHub authentication, secondmate liveness, secondmate convergence, pending handoff delivery, and project clone refresh with its drift reporting' ;;
...
EOF
  ...
  if [ "$downgraded" -eq 1 ]; then
    printf 'NETWORK_CHECKS: the fleet lock was no longer held by the session that requested these, so secondmate liveness, secondmate convergence, pending handoff delivery, and project clone refresh were skipped; they belong to whichever session holds the lock now\n' >> "$out"
  fi
  case "$rc" in
