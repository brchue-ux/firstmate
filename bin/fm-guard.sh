...
# (state/x-watch.check.sh exists) and supervision is not healthy, prints a loud,
# clearly delimited banner so the agent cannot skim past it in the tool output of
# whatever it was doing - the one channel every harness has. Supervision health
# is MODEL-AWARE (fm_watcher_supervision_verdict in bin/fm-wake-lib.sh): under
# the Claude Stop auto-arm model the watcher runs only between turns, so mid-turn
# a fresh beacon with no live watcher is healthy and only a stale beacon (beyond
# FM_GUARD_GRACE) is a genuine lapse; under the Pi extension model the extension
# tears the watcher down and respawns it on every actionable wake, so a fresh
# beacon with a genuinely unheld lock is healthy while that live Pi session
...
# watcher process vs a genuinely stale beacon).
#
...
# deduplicated hardest. The episode key is the failing condition, not the beacon
# mtime, which a healthy between-turns watcher advances every poll.
#
...
# source, or an X-mode relay poll needs supervision; count in-flight tasks so
# the banner can say how much is riding on an absent watcher.
