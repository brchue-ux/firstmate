...
# which no-mistakes version is on PATH, whether that no-mistakes advertises a
# newer release (and on which stream), which gh-axi version is on PATH, and
...
SH
  ...
  assert_contains "$out" "changed before secondmate liveness" \
...
test_network_phases_record_per_step_elapsed_times() {
  ...
  assert_timing_record "$log" phase secondmate-liveness '' "the secondmate liveness sweep was not timed"
