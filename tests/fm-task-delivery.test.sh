...
# Its output carries a third field, the project's base remote, which defaults to
# "origin" for every fixture here.
...
EOF
  ...
  [ "$out" = "no-mistakes off origin" ] || fail "conditional policy did not map to its most rigorous leg (got '$out')"
  ...
  [ "$out" = "no-mistakes on origin" ] || fail "conditional policy dropped its +yolo posture (got '$out')"

  out=$(FM_HOME="$home" "$PROJECT_MODE" --raw prodproj 2>/dev/null)
  [ "$out" = "no-mistakes-prod-only off origin" ] || fail "--raw did not expose the registered annotation (got '$out')"

  out=$(FM_HOME="$home" "$PROJECT_MODE" --raw flatproj 2>/dev/null)
  [ "$out" = "direct-PR off origin" ] || fail "--raw altered a flat registered mode (got '$out')"

  out=$(FM_HOME="$home" "$PROJECT_MODE" typoproj 2>/dev/null)
  [ "$out" = "no-mistakes off origin" ] || fail "a typo'd mode no longer falls back to the most rigorous default"
