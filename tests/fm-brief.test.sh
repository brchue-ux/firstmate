...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-nomistakes-a1:no-mistakes" "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    id=${id_mode%%:*}
    mode=${id_mode##*:}
    FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" some-proj --mode "$mode" --pr-repo owner/some-proj --pr-base main >/dev/null 2>&1; status=$?
...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" direct-proj --mode direct-PR --pr-repo owner/direct-proj --pr-base main >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
...
test_herdr_lab_contract_is_explicit_and_complete() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" firstmate --mode no-mistakes --herdr-lab --pr-repo owner/firstmate --pr-base main >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
...
test_herdr_lab_omission_is_loud_for_ship_and_scout() {
  ...
  for kind in ship scout; do
    ...
    if [ "$kind" = scout ]; then
      ...
      FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" firstmate --mode no-mistakes --pr-repo owner/firstmate --pr-base main >/dev/null 2>&1
    fi
    brief="$home/data/$id/brief.md"
    ...
    case "$kind" in
      ship)
        FM_HOME="$home" FM_CLASSIFY_PAUSED_VERB=awaiting \
          "$ROOT/bin/fm-brief.sh" "$id" firstmate --mode no-mistakes --pr-repo owner/firstmate --pr-base main >/dev/null 2>&1
        ;;
      scout)
        FM_HOME="$home" FM_CLASSIFY_PAUSED_VERB=awaiting \
