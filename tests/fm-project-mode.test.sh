...
test_spawn_records_uncorrupted_posture_for_a_base_remote_project() {
  ...
  # fm-spawn refreshes a pooled worktree from origin before launch and refuses to
  # start on a base it could not fetch, so the fixture needs a reachable origin.
  # A local bare repo is enough and keeps the case offline.
  git clone --quiet --bare "$proj" "$home/origin.git" || fail "fixture origin was not created"
  git -C "$proj" remote add origin "file://$home/origin.git" || fail "fixture origin was not registered"
  git -C "$proj" fetch --quiet origin || fail "fixture origin is not fetchable"
...
SH
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME="$home" \
    ...
    PATH="$fakebin:$PATH" \
    "$ROOT/bin/fm-spawn.sh" t1 "$proj" codex --mode direct-PR --yolo on 2>&1) \
