...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca when explicitly selected); kill the session anytime and the next one reconciles and carries on, never auto-relaunching a confirmed-dead secondmate agent - you reopen one yourself.
