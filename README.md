...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca when explicitly selected); kill the session anytime and the next one reconciles and carries on, relaunching a confirmed-dead secondmate agent only when that secondmate's own records still hold pending work - you reopen an idle one yourself.
