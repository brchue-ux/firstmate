...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>` - the session-start liveness sweep owes that secondmate a launch and could not deliver it, so that secondmate is not guaranteed live.
  Fleet startup launches the first mate and nothing else, so the sweep only ever owes a launch to a dead or missing LOCAL secondmate whose own durable records show pending work (`secondmate-provisioning` "Recovery" owns that test); a remote home's records are not readable from here, so a remote mate is relaunched without it.
  A registered local secondmate deliberately left down for having no pending work is correct and stays silent, so a line here always names a real gap: investigate the reason, and for a `cannot judge pending work` skip repair the named record - the recorded home for a missing, unsafe, or unresolvable one, its `data/backlog.md` for an unreadable backlog - before rerunning session start, because the sweep refused to guess rather than launching an idle secondmate or stranding a busy one.
