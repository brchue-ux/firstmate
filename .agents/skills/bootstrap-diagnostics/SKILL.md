...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>` - the session-start liveness sweep owes that secondmate a launch and could not deliver it.
  ...
  A registered secondmate deliberately left down for having no pending work is correct and stays silent, so a line here always names a real gap: investigate the reason, and for a `cannot judge pending work` skip fix the recorded home before rerunning session start, because the sweep refused to guess rather than launching an idle secondmate or stranding a busy one.
