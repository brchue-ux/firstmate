...
- `NO_MISTAKES_OUTDATED: v<installed> -> v<available> available (run: no-mistakes update -y)` - the installed no-mistakes clears the compatibility floor but is not the current release, reported from no-mistakes' own upgrade banner.
