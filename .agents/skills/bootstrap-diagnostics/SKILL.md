...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, NO_MISTAKES_OUTDATED, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, NETWORK_CHECKS, PR_CHECK_MIGRATION, SCRATCH_SWEEP, TMP_USAGE, BROWSER_SWEEP, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or when a standalone bin/fm-bootstrap.sh, bin/fm-startup-network.sh, or bin/fm-browser-sweep.sh run prints one of those lines.
...
Handle each printed line as below, before dispatching work that depends on it; `bin/fm-bootstrap.sh`'s header owns the line formats and this playbook owns the response.
...
- `NO_MISTAKES_OUTDATED: v<installed> -> v<available> available (run: no-mistakes update -y)` - the installed no-mistakes clears the compatibility floor but is not the current release, with the available release taken from no-mistakes' own upgrade banner.
  ...
  The installed version is read from the binary itself, not the banner, and the line is suppressed unless the advertised release is strictly newer.
...
- `SCRATCH_SWEEP: <name>: skipped: <reason>` - the temp-root scratch sweep found a stale orphaned scratch directory but deliberately left it in place, either because removal failed or because it does not look like scratch at all (`looks like a firstmate home` means a home is sitting in the temp root and must be moved or removed by hand, never swept).
  A leak that keeps being skipped eventually fills the shared temp filesystem (see `TMP_USAGE:` below), so clear the named directory yourself once you have confirmed nothing is using it.
...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>` - the session-start liveness sweep could not classify or safely converge that secondmate's recorded endpoint or home.
  Fleet startup never relaunches a secondmate, local or remote (never, regardless of pending work; the captain reopens one manually), so this line never means a launch was owed - it means an existing ambiguous process, an unreadable target, an unverified harness, or an unresolvable home blocked even the routine cleanup or FYI reporting.
  A dead or missing secondmate that was simply left down is correct and stays silent by default, so a line here always names a real gap: investigate the reason, and for a `cannot judge pending work` skip repair the named record - the recorded home for a missing, unsafe, or unresolvable one, its `data/backlog.md` for an unreadable backlog - before rerunning session start.
...
- `NUDGE_SECONDMATES: secondmate <id>: send failed: <reason>` - the secondmate sweep fast-forwarded a running secondmate home and its loaded instruction surface (`AGENTS.md`, `bin/`, or `.agents/skills/`) changed, but the deterministic `fm-send.sh fm-<id>` re-read nudge failed.
  ...
  Any marker it already had is retired rather than kept.
