...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>` - the session-start liveness sweep could not classify or safely converge that secondmate's recorded endpoint or home.
  Fleet startup never relaunches a secondmate (never, regardless of pending work; the captain reopens one manually), so this line never means a launch was owed - it means an existing ambiguous process, an unreadable target, an unverified harness, or an unresolvable home blocked even the routine cleanup or FYI reporting.
