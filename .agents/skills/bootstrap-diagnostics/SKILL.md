...
- `NUDGE_SECONDMATES: secondmate <id>: send failed: <reason>` - the secondmate sweep fast-forwarded a running secondmate home and its loaded instruction surface (`AGENTS.md`, `bin/`, or `.agents/skills/`) changed, but the deterministic `fm-send.sh fm-<id>` re-read nudge failed.
  ...
  A secondmate the liveness sweep deliberately left down never produces this line or a `CONFIG_REREAD:` one: its home is still fast-forwarded and still receives propagated config, but nothing is sent into an endpoint that is dead by design, and any marker it already had is left untouched until the captain reopens it.
