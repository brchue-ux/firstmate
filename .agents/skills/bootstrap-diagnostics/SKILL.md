...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>` - the session-start liveness sweep owes that secondmate a launch and could not deliver it.
  ...
  A registered secondmate deliberately left down for having no pending work is correct and stays silent, so a line here always names a real gap: investigate the reason, and for a `cannot judge pending work` skip repair the named record - the recorded home for a missing, unsafe, or unresolvable one, its `data/backlog.md` for an unreadable backlog - before rerunning session start.
