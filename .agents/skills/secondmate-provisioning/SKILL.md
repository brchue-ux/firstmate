...
The locked session-start deferred network stage runs the same bootstrap sweep for every local secondmate home it has a record for, discovered from `state/<id>.meta` records with `kind=secondmate` (`data/secondmates.md` only backfills `home=` for older records), including a home whose agent the liveness sweep deliberately left down under "Recovery".
